...
window.saveCurrentTrip = async function() {
  ...
  try {
    // Calculate total cost (handle if tripData isn't fully loaded)
    const totalCost = 
      (tripData?.flights?.reduce((sum, f) => sum + (f.price || 0), 0) || 0) +
      (tripData?.accommodation?.reduce((sum, a) => sum + (a.price || 0), 0) || 0) +
      (tripData?.activities?.reduce((sum, a) => sum + (a.price || 0), 0) || 0) +
      (tripData?.insurance?.reduce((sum, i) => sum + (i.price || 0), 0) || 0);
